...
// Fonction de lecture du registre Modbus
uint16_t ModbusCommunicator::readModbusRegister(unsigned char* request, int requestSize) {
    ...
    if (bytesRead >= 9 && !(response[7] & 0x80)) {
        return (response[9] << 8) | response[10];  // Renvoie les 2 octets du registre
    } else {
...
// Fonction d'écriture du registre Modbus
bool ModbusCommunicator::writeModbusRegister(unsigned char* request, int requestSize) {
    if (send(sock, (char*)request, requestSize, 0) == SOCKET_ERROR) {
        qDebug() << "Erreur d'envoi de la requête Modbus";
        return false;
    }
    ...
    // Vérifie si la fonction Modbus a réussi (fonction 0x06 pour écrire un registre)
    if (bytesRead >= 8 && response[7] == 0x06) {
